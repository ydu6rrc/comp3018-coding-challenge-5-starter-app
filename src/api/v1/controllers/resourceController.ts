...
export const getResourceById = (req: Request, res: Response): void => {
    const resource: Resource | undefined = resourceService.getResourceById(Number(req.params.id));
    if (resource) {
        res.status(HTTP_STATUS.OK).json({ message: "Resource found", data: resource });
    } else {
...
export const createResource = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!req.body.title) {
            ...
            res.status(HTTP_STATUS.BAD_REQUEST).json({
                message: "Invalid type. Must be one of: article, video, tutorial, documentation"
            })
        } else {
            ...
            const newResource: Resource = await resourceService.createResource(resourceData);
